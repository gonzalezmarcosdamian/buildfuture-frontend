  ...
  if (loading) {
    return (
      <div className="space-y-2">
        <div className="h-4 w-40 bg-bf-surface-2 rounded animate-pulse" />
        <div className="flex gap-3">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-44 w-52 shrink-0 bg-bf-surface-2 rounded-2xl animate-pulse" />
          ))}
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-sm font-semibold text-bf-text">Para vos mañana</h2>
          <p className="text-xs text-bf-text-3">{formatARS(capitalArs)} a invertir · objetivo libertad financiera</p>
        </div>
        <div className="flex gap-1">
          <button onClick={() => scroll("left")} className="p-1.5 rounded-lg bg-bf-surface-2 hover:bg-bf-surface-3 text-bf-text-3 transition-colors">
            <ChevronLeft size={14} />
          </button>
          <button onClick={() => scroll("right")} className="p-1.5 rounded-lg bg-bf-surface-2 hover:bg-bf-surface-3 text-bf-text-3 transition-colors">
      ...
      <div ref={scrollRef} className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x snap-mandatory">
        {recs.map((rec) => (
          <div
            key={rec.ticker}
            className="shrink-0 w-52 snap-start bg-bf-surface border border-bf-border rounded-2xl p-4 space-y-3"
          >
            {/* Header */}
            <div className="flex items-start justify-between">
              <div>
                <p className={`text-xs font-bold ${assetColors[rec.asset_type] || "text-bf-text-2"}`}>
                  {rec.ticker}
                </p>
                <p className="text-[11px] text-bf-text-3 leading-tight mt-0.5">{rec.name}</p>
              </div>
              {rec.already_in_portfolio && (
                <span className="text-[9px] bg-bf-surface-2 text-bf-text-3 px-1.5 py-0.5 rounded">ya tenés</span>
              )}
            </div>

            {/* Yield */}
            <div className="bg-bf-surface-2 rounded-xl px-3 py-2">
              <p className="text-[10px] text-bf-text-3">Rendimiento anual est.</p>
              <p className="text-lg font-bold text-emerald-400">{(rec.annual_yield_pct * 100).toFixed(2)}%</p>
              <p className="text-[10px] text-bf-text-3">{rec.currency === "USD" ? "en USD" : "en ARS"}</p>
            </div>

            {/* Allocation */}
            <div className="space-y-1">
              <div className="flex justify-between text-[10px]">
                <span className="text-bf-text-3">Monto sugerido</span>
                <span className="text-bf-text-2">{(rec.allocation_pct * 100).toFixed(2)}%</span>
              </div>
              <p className="text-xs font-medium text-bf-text-2">{formatARS(rec.amount_ars)}</p>
              <p className="text-[10px] text-bf-text-3">≈ USD {rec.amount_usd} → +${rec.monthly_return_usd}/mes</p>
            </div>
            ...
            {/* Rationale */}
            <p className="text-[10px] text-bf-text-3 leading-relaxed">{rec.rationale}</p>
          </div>
        ))}
      </div>
    </div>
